    ...
    Mutation: {
        tapCoin: async (_, { email }) => {
            ...
            try {
                ...
                if (!data) {
                    const { error: insertError } = await index_1.supabase
                        .from("users")
                        .insert([{ email: email, coins: 1 }]);
                    if (insertError) {
                        console.error(`Error inserting new user: ${insertError.message}`);
                        throw new Error(`Unable to insert new user: ${insertError.message}`);
                    }
                ...
                throw new Error("An unexpected error occurred while updating the coin count.");
            }
        },
    },
};
